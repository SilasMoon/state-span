...
export const GanttBar = React.memo(({
  ...
  // Position bars to overlap with grid lines on both sides
  // Left edge aligns with left grid line, right edge extends 1px to overlap with right grid line
  const left = (tempStart / zoom.hoursPerColumn) * columnWidth;
  const width = (tempDuration / zoom.hoursPerColumn) * columnWidth + 1;
  ...
  return (
    <>
      <div
        className={`absolute ${isSummary ? 'h-0.5' : isState ? 'h-[22px]' : 'h-6 rounded'} ${isSummary ? 'cursor-default' : isDragging === 'move' ? 'cursor-grabbing' : isModifierPressed ? 'cursor-crosshair' : 'cursor-grab'} group flex items-center justify-center text-xs ${isState ? '' : 'font-medium'} ${isSummary ? '' : 'shadow-lg hover:shadow-xl'} transition-all pointer-events-auto ${
          isSelected ? 'ring-2 ring-primary ring-offset-2 z-30' : isState ? 'z-30' : 'z-10'
      ...
      {isModifierPressed && !isSummary && (
        <>
          ...
          <div
            ...
            style={{
              left: `${left - 10}px`, // Center handle on left edge of bar
          ...
          <div
            ...
            style={{
              left: `${left + width - 10}px`, // Center handle on right edge of bar
              ...(isState ? { top: '50%' } : { top: '50%' }),
              transform: 'translateY(-50%)',
            }}
